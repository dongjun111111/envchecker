...
func (s *Obj_Apm) CheckObj(objcfg *config.ObjCfg) (res []byte) {
	...
	if objcfg.Link == "" {
		return s.OutPut(nil, errors.New("empty apm link"))
	}
	trans, err := transport.NewHTTPTransport()
	if err != nil {
		log.Println("tracer create transport failed.", err)
		return s.OutPut(nil, errors.New("tracer create transport failed."+err.Error()))
	}
	u, err := url.Parse(objcfg.Link)
	if err != nil {
		log.Println("get apm-server error.", err)
		return s.OutPut(nil, errors.New(" get apm-server error."+err.Error()))
	}
